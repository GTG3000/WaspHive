...
function closeDoor()
  if storage.state ~= false then
	...
	local tdloc = {}
	tdloc.poly  = { {-1.1, 0.0 }, {-1.1, 5.0}, {1.1, 5.0}, {1.1, 0.0} }
	tdloc.damage  = 50
	tdloc.damageType = "IgnoresDef"
	tdloc.damageSourceKind = "electric"
	--object.setDamageSources({touchDamage={poly  = { {-1.1, 0.0 }, {-1.1, 5.0}, {1.1, 5.0}, {1.1, 0.0} },damage  = 50,damageType = "IgnoresDef",damageSourceKind = "electric"}})
	--object.setDamageSources("\"touchDamage\":[\"poly\":[[-1.1,0.0],[-1.1,5.0],[1.1,5.0],[1.1,0.0]],\"damage\":50,\"damageType\"=\"IgnoresDef\",damageSourceKind=\"electric\"]")
	object.setDamageSources(tdloc)
    updateCollisionAndWires()
    updateLight()
  end
end
